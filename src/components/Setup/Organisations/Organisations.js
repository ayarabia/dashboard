...
const Organisations = () => {
  ...
  const handleUpdate = (item) => {
    ...
    setupdateData({
      ...
      person_in_charge_name: item.person_in_charge_name,
      phone: item.phone,
    ...
    <div className="flex-grow-1 bg-main-content py-5 px-3 overflow-hidden">
              ...
              <div className="modal-body">
                ...
                <form>
                  ...
                  <div className="input-group mb-3 align-items-center">
                    <label className="me-3" htmlFor="inputGroupSelect01">
                      Options:
                    </label>
                  ...
                  <button
                    type="submit"
                    className="approve-btn p-2 px-3 rounded-3"
                  >
                    <i className="fa-solid fa-check  me-3"></i> Submit
                  </button>
                </form>
              </div>
            </div>
              ...
              <div className="modal-body">
                ...
                <form>
                  ...
                  <button
                    ...
                    className="approve-btn p-2 px-3 rounded-3"
                  >
                    <i className="fa-solid fa-arrow-rotate-right me-3"></i>{" "}
                    Reset Filters
                  </button>
                </form>
              </div>
      ...
      <div className="d-flex justify-content-between mt-2 align-items-center">
        <div className="d-flex">
          <button
            className="add-btn shadow-sm py-1 px-3"
            data-bs-toggle="modal"
            data-bs-target="#addOrgModal"
          >
            <i className="fas fa-plus"></i> Add New Organization
          </button>
          <button className="delete-btn shadow-sm py-1 px-3 ms-3">
      ...
      <div className="mt-3 position-relative">
        <div className="overflow-auto">
          <table className="w-100 ">
            ...
            <tbody>
              {organisationsData &&
                organisationsData.map((item, idx) => (
                  <tr key={idx}>
                    <td>{item.id}</td>
                    <td>
                      <input
                    ...
                    <td>
                      ...
                      <div className="organization__icons">
                        ...
                        <EditOrgModal
                          deleteId={deleteId}
                          getOrganisationsData={getOrganisationsData}
                        ...
                        <DeleteModal
                          id={deleteId}
                          deleteData={{
                            type: "an organisation",
                            content: "Lorem, ipsum.",
                            deletePoint: `organizations`,
                          }}
                          fetchData={getOrganisationsData}
                        />
                        <Link
                          className="text-black"
                          to={"/dashboard/manage-users/user-permissions"}
                        >
                    ...
                    <td>{item.address}</td>
                    <td>{item.phone}</td>
                    <td>{item.country_partners_count} </td>
                    <td>{item.person_in_charge_name}</td>
                    <td>{item.billing_address}</td>
                    <td>{item.shipping_address}</td>
                    <td>
                      {item.created_by} at {item.created_at}
                    </td>
                    <td>
                      {item.updated_by} at {item.updated_at}
                    </td>
                  </tr>
                ))}
            </tbody>
